...
class DQNAgent:
    ...
    def populate_replay_mem(self, env):
        ...
        for iter in range(self.replay_start_size):
            ...
            if iter<self.held_out_states_size:
                self.held_out_states.append(self.preprocessor.process_state_for_memory(s_t),a_t,r_t, self.preprocessor.process_state_for_memory(s_t1), is_terminal)
    ...
    def save_weights_on_interval(self, curiter, totaliter):
        ...
        if (curiter == 0):
            ...
            self.q_network.save_weights("model0.h5")
            ...
            with open("model1.json", "w") as json_file:
                json_file.write(model_json)
            # serialize weights to HDF5
            self.q_network.save_weights("model1.h5")
            ...
            with open("model2.json", "w") as json_file:
                json_file.write(model_json)
            # serialize weights to HDF5
            self.q_network.save_weights("model2.h5")
            ...
            with open("model3.json", "w") as json_file:
                json_file.write(model_json)
            # serialize weights to HDF5
            self.q_network.save_weights("model3.h5")
