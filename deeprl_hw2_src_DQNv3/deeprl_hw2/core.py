...
class Sample:
    ...
    def __init__(self,s_t,a_t,r_t,s_t1,is_terminal):
        #assert(((s_t)[0][0][0]).dtype=="uint8")
        #assert(((s_t1)[0][0][0]).dtype=="uint8")
        
        self.states = list()
        self.states.append(s_t[0])
        for i in range(4):
            self.states.append(s_t1[i])
...
class Preprocessor:
    ...
    def process_batch(self, samples):
        ...
        """
           #new samples list to store "deep copies" of samples taken               
        #from the replay memory                                                 
        newSamples = list()

        # make compute state inputs and targets                                 
        for sample in samples:
            #Note: np.float64 returns a deep copy                               
            states = np.float64(sample.states)

            #create a new sample object such that the reffernce                 
            #to the sample object taken from the replay memory                  
            #isn't returned                                                     
            newSamples.append(Sample(states[0:3],sample.a_t,sample.r_t,states[1:5],sample.is_terminal))
...
class ReplayMemory:
    ...
    def sample(self, batch_size, indexes=None):
        samples = random.sample(list(self.M), batch_size)
