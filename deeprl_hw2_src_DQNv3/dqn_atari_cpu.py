...
def create_model(window, input_shape, num_actions,
    ...
    model = Sequential()
    model.add(Convolution2D(32, 8, 8, subsample=(4, 4), kernel_initializer="normal",
                            border_mode='same', input_shape=(window,input_shape[0],input_shape[1])))
    model.add(Activation('relu'))
    model.add(Convolution2D(64, 4, 4, subsample=(2, 2), kernel_initializer="normal",
                            border_mode='same'))
    model.add(Activation('relu'))
    model.add(Convolution2D(64, 3, 3, subsample=(1, 1), kernel_initializer="normal",
                            border_mode='same'))
    model.add(Activation('relu'))
    model.add(Flatten())
...
def main():  # noqa: D103
    ...
    agent = DQNAgent(model,target,preprocessor,history,memory,policy,held_out_states,HELD_OUT_STATES_SIZE,GAMMA,TARGET_UPDATE_FREQ,NUM_BURN_IN,TRAIN_FREQ,BATCH_SIZE,REPLAY_START_SIZE,NUM_ACTIONS)
