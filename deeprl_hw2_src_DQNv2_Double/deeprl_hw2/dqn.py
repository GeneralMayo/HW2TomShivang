    ...
    def update_policy(self):
        ...
        for sampleIdx in range(self.batch_size):
          ...
          if(is_terminal):
            ...
            if(self.is_double_q):
                #get optimal action using online network
                q_t1_online = self.calc_q_values(s_t1)
                a_t1_online = np.argmax(q_t1_online[0])
                #get optimal qvalue using target network
                q_t1_target = self.target_q_network.predict(s_t1)
                #store target
                targets[sampleIdx][a_t] = self.gamma*q_t1_target[0][a_t1_online] + r_t
